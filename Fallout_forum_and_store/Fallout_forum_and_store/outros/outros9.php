...
include '../cnn1.php';
$id = 9;
$sql = "SELECT nome, preco , quantidade, descricao, imagem FROM outros WHERE id_outros = :id";
try {
...
$sql_venda = "INSERT INTO vendas (id_usuario, produto_comprado, quantidade, preco_total)
VALUES (:id_usuario, :produto_comprado, :quantidade, :preco_total)";
$stmt_venda = $pdo->prepare($sql_venda);
$stmt_venda->bindParam(':id_usuario', $id_usuario, PDO::PARAM_INT);
$stmt_venda->bindParam(':produto_comprado', $nome_produto, PDO::PARAM_STR);
$stmt_venda->bindParam(':quantidade', $quantidade_venda, PDO::PARAM_INT);
$preco_total = $preco_produto * $quantidade_venda;
$stmt_venda->bindParam(':preco_total', $preco_total);
$stmt_venda->execute();


$sql_estoque = "UPDATE outros SET quantidade = quantidade - :quantidade WHERE id_outros = :id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar_produto'])) {
    ...
    try {
        $sql = "DELETE FROM outros WHERE id_outros = :id";
        $stmt = $pdo->prepare($sql);
...
<div class="con">
    ...
    <div class="product-page">
        <div class="product-images">
            <img src="<?php echo $caminho_imagem; ?>" style="height:400px;width: 400px" alt="Imagem do Produto">
        </div>
        ...
        <div class="product-details">
            ...
            <div class="btn-group">
                <a class="btn" style="text-effect: none;color: white;" href="../alteraroutro.php?id_produto=9">Alteração do Produto</a>
        ...
        try {
            $sql = "SELECT nome, imagem, preco, caminho FROM outros WHERE id_outros IN (6, 7, 5) LIMIT 4";
            $stmt = $pdo->query($sql);
                    ...
                    <div class="related-items">
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
                            <div class="related-item">
                                <a href="<?php echo '../' . htmlspecialchars($row['caminho']); ?>">
                                    <img src="<?php echo '../' . htmlspecialchars($row['imagem']); ?>" height="150px" alt="<?php echo htmlspecialchars($row['nome']); ?>">
                                </a>
                                <h3><?php echo htmlspecialchars($row['nome']); ?></h3>
                                <div class="price">€ <?php echo number_format($row['preco'], 2, ',', '.'); ?></div>
                            </div>
                        <?php } ?>
                    </div>
                </div>
