...
if (!isset($_SESSION['carrinho']) || empty($_SESSION['carrinho'])) {
    echo "<h1 style='text-align: center;font-size: 70px'>O carrinho está vazio!</h1>";
    echo "<div style='display: flex; justify-content: center; height: 50vh;'>
        <img src='imagens_loja/empty-cart.png'>
      </div>";
    echo "<a href='loja_index.html'>
          <div style='display: flex; justify-content: center; height: 50vh;'>
          <button style='height: 5vh;width: 50vh;border-radius: 20px;padding: 12px 20px;background-color: #ff8c00;color: white;font-size: 16px; font-weight: bold;border: none;cursor: pointer;transition: background-color 0.3s;'>Voltar a Loja!</button></a>
          </div>";
    exit;
}
...
<h1 style="margin-top: 100px;color: black">Carrinho de Compras</h1>
<table border="1" style="width: 100%; border-collapse: collapse; background-color: white; color: black;">
    <tr>
        <th style="padding: 10px; background-color: #ff8c00; color: white;">Produto</th>
        <th style="padding: 10px; background-color: #ff8c00; color: white;">Preço</th>
        <th style="padding: 10px; background-color: #ff8c00; color: white;">Quantidade</th>
        <th style="padding: 10px; background-color: #ff8c00; color: white;">Ações</th>
    </tr>
    <?php foreach ($_SESSION['carrinho'] as $index => $item): ?>
        <tr style="text-align: center; color: black; background-color: white;">
            <td style="padding: 10px;"><?php echo htmlspecialchars($item['nome']); ?></td>
            <td style="padding: 10px;">€ <?php echo number_format($item['preco'], 2, ',', '.'); ?></td>
            <td style="padding: 10px;"><?php echo htmlspecialchars($item['quantidade']); ?></td>
            <td style="padding: 10px;">
                <form method="post" action="">
                    <input type="hidden" name="index" value="<?php echo $index; ?>">
                    <button type="submit" name="remover" style="padding: 5px 10px; background-color: #ff8c00; color: white; border: none; border-radius: 5px; cursor: pointer;">Remover Produto</button>
                </form>
            </td>
        </tr>
    <?php endforeach; ?>
...
<form method="post" action="finalizar_compra.php">
    <button type="submit" id="finalizar-btn" name="finalizar_compra">Finalizar Compra</button>
</form>
</div>
<div id="msg-card" class="msg-card" style="display: none;">
    <div class="msg-card-content">
        <p id="msg">Compra finalizada com sucesso!</p>
    </div>
</div>
...
<script src="bootstrap-5.3.3-dist/js/carrinho.js"></script>
</body>
</html>
