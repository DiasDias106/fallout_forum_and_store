...
try {
    $stmt = $pdo->query("SELECT * FROM forca");
...
if (isset($_GET['remover_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM forca WHERE id_jogo = ?");
        $stmt->execute([$id]);
...
<main class="modular">
    ...
    <table id="jogosTable">
        ...
        <tr>
            ...
            <th>Letra Escolhida</th>
            <th>Palavra certa</th>
            <th>Venceu</th>
            <?php if (!empty($_SESSION['cargo']) && $_SESSION['cargo'] === 'admin' || $_SESSION['username'] === 'administrador'): ?>
                <th>Ações</th>
            <?php endif; ?>
        </tr>
        </thead>
        ...
        <?php foreach ($jogos as $jogo): ?>
            <tr>
                ...
                <td><?= htmlspecialchars($jogo['letra_escolhida']) ?></td>
                <td><?= htmlspecialchars($jogo['palavra_certa']) ?></td>
                <td><?= $jogo['venceu'] ? 'Sim' : 'Não' ?></td>
                <?php if (!empty($_SESSION['cargo']) && $_SESSION['cargo'] === 'admin' || $_SESSION['username'] === 'administrador'): ?>
                    <td>
                        <a href="?remover_id=<?= $jogo['id_jogo'] ?>" onclick="return confirm('Deseja remover este registro?')">
                            <button>Remover</button>
                        </a>
                        <a href="editar_jogo.php?id=<?= $jogo['id_jogo'] ?>">
    ...
    <a href="jogoforca.php"><button>Voltar ao Jogo</button></a>
</main>
