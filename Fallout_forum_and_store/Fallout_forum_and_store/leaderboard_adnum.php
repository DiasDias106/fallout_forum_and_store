...
try {
    $stmt = $pdo->query("SELECT * FROM adivinhanumero ORDER BY id_jogo asc");
...
if (isset($_GET['remover_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM adivinhanumero WHERE id_jogo = ?");
        $stmt->execute([$id]);
...
<main class="modular">
    ...
    <table id="jogosTable">
        ...
        <?php foreach ($jogos as $jogo): ?>
            <tr>
                ...
                <td><?= htmlspecialchars($jogo['numero_escolhido']) ?></td>
                <td><?= htmlspecialchars($jogo['numero_correto']) ?></td>
                <td><?= htmlspecialchars($jogo['tentativas']) ?></td>
                <?php if ($_SESSION['cargo'] == 'admin'): ?>
                    <td>
    ...
    <a href="adivinhanum.php"><button>Voltar ao Jogo</button></a>
</main>
